...
function SalesChart({bookings, numDays}) {
  ...
  const data = allDates.map(date => {
    return {
      label: format(date, 'MMM dd'), //Month Day
      totalSales: bookings.filter(booking => isSameDay(date, new Date(booking.created_at))
      ).reduce((acc, current) => acc + current.totalPrice, 0), //method from library 
  ...
  return <StyledSalesChart>
    ...
    <ResponsiveContainer height={300} width='100%'>
      <AreaChart data={data} >
        ...
        <Area 
           dataKey='totalSales' type='monotone'
          ...
          />
          <Area 
           dataKey='extrasSales' type='monotone'
           stroke={colors.extrasSales.stroke}
           fill={colors.extrasSales.fill}
           strokeWidth={2}
           name="Extra sales sales"
           unit="$"
          />
